...
| **CapEx — efficient (heat pump)** | **NOT FOUND** | Web search turned up only "contact for quote" B2B listings (Alibaba, Made-in-China) with no visible pricing at 40kW or 150kW. The only real number found was a generic European €250-2,000/kW installed rule of thumb, which is residential-skewed and too wide to use as-is | Open flag — needs a distributor quote, a spec-sheet PDF with list price, or acceptance of the wide European range as a placeholder |
| **CapEx — baseline (mazut/diesel oven)** | **NOT FOUND** | Same problem, worse — search results were either small residential single units or large multi-MW industrial boilers, nothing at this specific 40-150kW commercial scale | Open flag — same as above |
