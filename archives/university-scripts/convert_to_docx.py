...
p8 = doc.add_paragraph(
    'For dairy milk, raw materials include cattle feed (forages such as alfalfa and ryegrass, grains such as corn and barley, and byproducts such as soybean meal and cotton seeds), water (approximately 6–8 liters per liter of milk produced, accounting for drinking water and water used in processing), and minerals and vitamins used as feed supplements. Additionally, this stage includes the energy required for farm operations.'
)

p9 = doc.add_paragraph(
    'For soy milk, raw materials consist of soybeans that are grown, harvested, and transported, water (27.7 liters per liter of soy milk produced), and minerals/additives. This stage also includes energy for crop cultivation.'
)
...
p12 = doc.add_paragraph(
    'For soy milk, processing begins with soaking dried soybeans in water. The soaked beans are then ground mechanically into a slurry, which is brought to a boil (15–20 minutes) to inactivate soybean trypsin inhibitor and sterilize the product. Following boiling, insoluble residues (soy pulp) are removed by straining and filtration. Chemical additives, specifically defoaming agents (typically glycerin-based fatty acid esters), are used during the boiling step to manage foam formation.'
)
...
p18 = doc.add_paragraph(
    'This analysis has attempted to include every major process within the defined system boundary. However, one significant process was included that could reasonably be excluded: '
)
p18_run = p18.add_run('retail store refrigeration energy')
p18_run.bold = True
p18.add_run('. While retail refrigeration is part of the distribution system, some LCA studies exclude retailer-specific energy on the grounds that retailers\' refrigeration systems serve multiple products and attributing a specific share to any single product is complex and imprecise. In this study, retail refrigeration is included because both dairy and soy milk require identical cold-chain management at retail, and this shared requirement is material to understanding the comparison. The environmental impact of retail refrigeration is likely negligible relative to the major processing and transport impacts for each system, but it is justifiable to include for completeness.')
...
p19 = doc.add_paragraph('Functional unit: One round-trip commute from Tel Aviv to Reichman University in Herzliya and back to Tel Aviv, representing a typical student journey to campus.')

doc.add_heading('Take-away coffee', level=3)
p20 = doc.add_paragraph('Functional unit: One 200-milliliter serving of coffee, delivered in a consumable form.')

doc.add_heading('Shopping bags', level=3)
p21 = doc.add_paragraph('Functional unit: One plastic shopping bag.')

doc.add_heading('Producing electricity', level=3)
p22 = doc.add_paragraph('Functional unit: One megawatt-hour (MWh) of electricity delivered to the grid.')

# Save document
doc.save(r'c:\עמרי\Shmags 2\projects\university\notes\HW2_Functional_Unit_and_System_Boundary.docx')
